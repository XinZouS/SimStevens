...
class GameScene: SKScene{
    ...
    var tipsNode: SKLabelNode!
    var tipsIsOn: Bool = false
    var tipsOrder: Int = 0
    ...
    func setUpBuildings(){
        ...
        babbio = Building(image: "sign-newhouse", name: "Babbio Center", year: 2006, order: 5, numOfStudents: 2200, upGradeCost: 9100, x: mapX+631, y: mapY-230, width: sd, height: sd)
        
        ship = Building(image: "sign-newhouse", name: "Ship", year: 1833, order: 6, numOfStudents: 100, upGradeCost: 100, x: mapX + 490, y: mapY + 330, width: sd, height: sd)
    ...
    func setUpButtons(){
        ...
        tipsNode = SKLabelNode(text: tipsLogo)
        tipsNode.fontSize = 40
        tipsNode.position = CGPoint(x:  -self.frame.midX - 330, y: -self.frame.midY - 50)
        tipsNode.zPosition = LayerAt.ButtonLabels.rawValue
    ...
    func setUpTeaching() { // the new player guide, only run once. ---------------------
        ...
        if teachingStep == 1 { // show welcome box, game info and task: build gatehouse.
            ...
            infoBox.addTextLabel(text: "OK, now let's build first Building⏩", color: clr, fontSize: sz, fontName: textFontName, atPosition: CGPoint(x: midX+60, y: maxY - base - offset * 4))
...
//            arrow.run(action, completion: { arrow.removeFromParent() })
            ...
            infoBox.addTextLabel(text: "🏆 Congradulations! 🏆", color: UIColor.blue, fontSize: 36, fontName: textFontNameBold, atPosition: wordPos)
...
//        print("currY: \(currentPosition.y), hiddingline=\(hiddingLine), maxX=\(self.frame.maxX)")
    ...
    func showInfoBoxOf(currentBuilding: Building){
        ...
        let imgPosition = CGPoint(x: infoBox.node.frame.minX+120, y: infoBox.node.frame.midY-60)
...
//        print("selected Building order: \(selectedBuilding.order), current: \(currentOrder)")
        // Yes upgrade:
    ...
    func showResearchingBoxOf(currentBuilding: Building) {
        ...
        let imgsize = CGSize(width: 230, height: 260)
        ...
        let textposition = CGPoint(x: infoBox.node.frame.midX+90, y: infoBox.node.frame.maxY-150)
...
//        infoBox.addButton()
    ...
    func doResearchInCurrentBuilding() {
        ...
        if selectedBuilding.timeToFinish == 0 { // not doing research, then start.
            ...
            let talkPos = CGPoint(x: self.frame.midX + 30, y: self.frame.midY + 3) // y ?= 33
            let wordPos = CGPoint(x: talkingBox.node.frame.midX, y: talkingBox.node.frame.midY)
            talkingBox.addTextLabel(text: "Oops! You need to wait for current research finish. ", color: UIColor.black, fontSize: 23, fontName: textFontName, atPosition: talkPos)
            talkingBox.moveBoxAtTop(toshow: true, offSet: talkingBoxOffset)
        }
        infoBox.moveBoxAtBottom(toshow: false, offSet: self.frame.maxY)
    }
    ...
    // for tips talking box: 
    func setupDefaultTalking(words:String) {
        talkingBox.node.removeAllChildren()
        
        let clr = UIColor.black
        let talkPosi = CGPoint(x: self.frame.midX + 30, y: self.frame.midY + 15) // y=33
        let wordInLn = 36  // for talking box paragraph.
        let fontSize: CGFloat = 27 // for talking box paragraph.

        talkingBox.addParagraph(pText: words, numOfCharInLine: wordInLn, color: clr, fontSize: fontSize, fontName: textFontName, initPosition: talkPosi)
    }
    // tips talking box setting:
    func showTips() { // 0: not show; 1-last: show steps;
        if tipsOrder == 1 {
            setupDefaultTalking(words: "\(helpLogo) helps you navigate to building. ")
            talkingBox.moveBoxAtTop(toshow: true, offSet: talkingBoxOffset)
        }
        else if tipsOrder == 2 {
            setupDefaultTalking(words: "\(showNameLogo) to show name of buildings. ")
        }
        else if tipsOrder == 3 {
            setupDefaultTalking(words: "\(professorLogo) is the population on Stevens. ")
        }
        else if tipsOrder == 4 {
            setupDefaultTalking(words: "\(moneyLogo) is money you have. ")
        }
        else if tipsOrder == 5 {
            setupDefaultTalking(words: "Find a treasure box on map to win \(moneyLogo) ")
        }
        else if tipsOrder == 6 {
            setupDefaultTalking(words: "In each building, you can do research to earn \(moneyLogo) ")
        }
        else if tipsOrder == 7 {
            setupDefaultTalking(words: "Task: Let's try to find a ship and build it. ")
        }
        else if tipsOrder == 8 {
            tipsOrder = -1 // close tips, move order back to 0.
            tipsIsOn = false
            talkingBox.moveBoxAtTop(toshow: false, offSet: talkingBoxOffset)
        }
//        print("tips Order = \(tipsOrder)")
        tipsOrder += 1
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if tipsOrder > 0 {
            showTips()
            return  // not allow user action before tips finishe showing.
        }
        ...
        for touch: AnyObject in touches {
            ...
            else if tipsNode.contains(locationOnScreen) {
                if tipsIsOn {
                    tipsIsOn = false
                    tipsOrder = 0
                    talkingBox.moveBoxAtTop(toshow: false, offSet: talkingBoxOffset)
                }else{
                    tipsIsOn = true // open tips in talking Box
                    tipsOrder = 1   // start show tips from 1.
                    showTips()
                }
            }
                // Interface buttons:-----------------------------------------------------------
